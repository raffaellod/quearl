...
Date.prototype.format = function $Date$format(sFormat) {
	return sFormat.replace(/%[crRxX]/g, (function($0, ich, s) {
		switch ($0) {
			case "%c": return Loc.QL_LOC_FMT_TS_DATE_S + " " + Loc.QL_LOC_FMT_TS_TIME_S;
			case "%r": return "%i" + Loc.QL_LOC_FMT_TS_TIME_S.substr(2, 1) + "%M %p";
			case "%R": return "%h" + Loc.QL_LOC_FMT_TS_TIME_S.substr(2, 1) + "%M";
			case "%x": return Loc.QL_LOC_FMT_TS_DATE_S;
			case "%X": return Loc.QL_LOC_FMT_TS_TIME_S;
		}
...
(function() {
	...
	function $Number$format(v, cDecs /*= 0*/) {
		...
		for (var i = s.length - 3; i > 0; i -= 3)
			s = s.substr(0, i) + Loc.QL_LOC_NUM_THOUSEP + s.substr(i);
		if (cDecs > 0) {
			...
			s += Loc.QL_LOC_NUM_DECSEP + (v.toString().match(reDecs) || {1: ""})[1].pad(cDecs, "0", 1);
		}
		return s;
	}
...
function $Number$formatCy(i) {
	return Number.format(i, Loc.QL_LOC_CUR_DECS);
}
